...
public abstract class AbstractStateStorer extends Component implements StateStorer {
  ...
  public AbstractStateStorer(Plugin plugin) {
    this.plugin = plugin;
  }
  ...
  public void storeHere(MultiblockState state) {
    state.getAllBlocksLocs().stream().map(AbstractStateStorer::normalize).forEach(loc -> stateByLocation.put(loc, state));
